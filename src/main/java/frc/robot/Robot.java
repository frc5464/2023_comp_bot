...
public class Robot extends TimedRobot {
  ...
  private static final String kScorePickup = "ScorePickup";
  ...
  public void robotInit() {
    ...
    m_chooser.addOption("ScorePickup", kScorePickup);
  ...
  public void setupAutoVals() {
    ...
    SmartDashboard.putNumber("sideStrafeAfterCubeTime", 0.5);
    SmartDashboard.putNumber("sideFormationTime", 1.3);
  ...
  public void scorePrep() {
    ...
    // // INCREASE this value to make us home faster, but possibly less stable.
    // Decrease if overshooting.
    // double divisor = 50;

    // // How wide of a range are we going to be looking for when homing? DECREASE
    // to look for smaller window.
    // double window = 3;
  ...
  public void HitchSlow() {
    ...
    if (drivetrain.frontleftrotations > 70) {
      // TODO: instead of encoders why not try an apriltag from further away to home?
      autoStep++;
    }
  }
  ...
  public void PickupEscape(){
    double rotate = drivetrain.SnapToAngle(gyro.Yaw, 0);

    drivetrain.Move(-0.3, 0, rotate);
    if (drivetrain.frontleftrotations < -60) {
      TargetYaw = gyro.Yaw;
      elevator.setElevatorPosition("Drive");
      autoTimer.start();
      wait.start();
  ...
  public void SideStrafeAfterAquiringaCube() {
    ...
    switch (autonomous_direction_selected) {
      case kLeft:
        rotate = drivetrain.SnapToAngle(gyro.Yaw, 15);
  ...
  public void Arrival() {
    ...
    // TODO: low: Get tag 7 to actually read out with the vision system, or switch
    // this to Gyro lineup.
  ...
  public void Balance() {
    ...
    // TODO: Test this out and see if our distance changes enough to be useful for
    // balancing
    // Elevator extension and ultrasonic voltage output can both be calibrated to do
    // this.
  ...
  public void SweeptheHouse() {
    ...
    // This is accomplished by adding 360 in the situation of a negative yaw.
    double yawWeDoBeUsing;
  ...
  public void SweepSnap() {
    // This function will use a transformed yaw from the previous function to snap
    // to an angle.
    // It will transform all negative yaws up by 360 to get a good continuous
    // homing.
    double yawWeDoBeUsing;
  ...
  // It then drives us onto the charging station, keeping us there with a
  // gyro/brake
  ...
  public void stickControlDrivetrain() {
    ...
    // Snap to 180 degrees for charge station climb, on far side of station
      ...
      rotate = drivetrain.SnapToAngle(yawWeDoBeUsing, 180);
    ...
    if (Fieldoriented == true) {
      // drivetrain.MoveFieldOriented(fwdBack * speed, rotate, leftRight * speed,
      // gyro.Angle);
      drivetrain.Move(fwdBack * speed, leftRight * speed, rotate);
    }
  }
  ...
  public void stickControlPidHoming() {
    ...
    // holds the elevator according to an auto-control scheme that is not as cool as
    // PID
    elevator.pidHoming();
  }
